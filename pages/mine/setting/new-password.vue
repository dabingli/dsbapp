...
<template>
	<view class="new-password">
		<view class="top">
			<view class="title">请输入新登录密码</view>
		</view>
		<view class="health">
			<view class="input">
				<input placeholder="6-16位密码" :password="showPassword" placeholder-style="input-placeholder" v-model="password" />
				<view class="iconfont" :class="[!showPassword ? 'icon-yanjing' : 'icon-biyanjing']" @click="changePassword(0)"></view>
			</view>
			<view class="input">
				<input placeholder="请再次输入登录密码" :password="showPasswordTwo" placeholder-style="input-placeholder" v-model="repassword" />
