<template>
	<view>
		<view class="card-list">
			<view class="card" v-for="(card,index) in list" :key="index" :class="card.perfect==1?'card-active':''">
				<view class="up">
					<view class="img">
				...
				<view class="down">
					...
					<view class="subscribe">
						...
						<button class="button" v-if="card.perfect==1" @click="addPlan(card.id)">预约还款</button>
...
<script>
	export default {
		...
		methods: {
			link(id){
				uni.navigateTo({
					url: `/pages/index/card-keeper/plan-list?cardid=${id}&plantype=2`
				})
			},
			// 获取信用卡列表
			getData(){
			...
			addPlan(id){
				this.id = id
				this.$store.dispatch('getPerfectAuth',{
					cardid: id,
							...
							success: (d)=>{
								if(d.confirm){
									...
									this.$store.dispatch('setCreditCardTwo',data).then(res=>{
										uni.navigateTo({
											url:`/pages/index/card-keeper/perfect-book?id=${this.id}`
										})
									}).catch(err=>{})
								}else{
									uni.showToast({
					...
					if(err.code==2){
						...
						uni.navigateTo({
							url:`/pages/index/card-keeper/plan-list?cardid=${this.id}&plantype=2`
						})
					}else if (res.code == 4){
						uni.showToast({
							title: '暂不支持此银行',
