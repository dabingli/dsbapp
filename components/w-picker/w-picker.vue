		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			<view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
			  <view class="w-picker-btn" @tap="pickerCancel">取消</view>
				<view v-if="unLimited" @tap="unLimit">不限</view>
	...
	export default {
		...
		props:{
			mode:{
				type:String,
				validator(mode){
					let modeList=['half','date', 'dateTime', 'yearMonth','time','region','selector','limit','range'];//过滤无效mode;
				...
				switch(mode){
					case "limit":
						...
						if(col1.value!=checkArr[0].value){
							let hours=initPicker.initLimitHours(_this.starHour,_this.endHour,_this.minuteStep,_this.afterStep,col1.value);
							let minutes=initPicker.initLimitMinutes(_this.starHour,_this.endHour,_this.minuteStep,_this.afterStep,col1.value,col2.value);
						...
						let d=_this.data.date[arr[0]]||_this.data.date[_this.data.date.length-1];
						let h=_this.data.hours[arr[1]]||_this.data.hours[_this.data.hours.length-1];
						let m=_this.data.minutes[arr[2]]||_this.data.minutes[_this.data.minutes.length-1];
					...
					case "half":
						...
						if(year!=checkArr[0]){
							days=initPicker.initDays(year,month,_this.disabledAfter);
							months=initPicker.initMonths(year,_this.disabledAfter);
			...
			initData(){
				...
				switch(mode){
					...
					case "range":
						...
						break;
					default:
						data=initPicker.date(_this.startYear,_this.endYear,_this.mode,_this.step,_this.useCurrent(),_this.current,_this.disabledAfter);
						break;
				}
				_this.data=data;
